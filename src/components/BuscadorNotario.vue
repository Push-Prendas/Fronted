	...
	if(tipo_solicitud == "I" || tipo_solicitud == ""){
		...
		await getDocs(collection(db, "Solicitud_Inscripcion_Prenda")).then((per_data) => {
			...
			my_docs.forEach((d) => {
                ...
                var Notario = "-1"
                var Creador = "-2"
               getDocs(collection(db, "Usuario")).then((data) => {
                data.docs.forEach((jj) => {

                if(my_sol.usuarioCreador == jj.data().mail){
                    Creador = jj.data().NotariaID

                }
                if(localStorage.mail == jj.data().mail){
   
                    Notario =  jj.data().NotariaID
                }

                })
            }).then(() =>{
                if(my_sol.estadoSecundario > 0 && Creador == Notario){
